...
func _physics_process(_delta: float) -> void:
	...
	var distance =GameEvent.play_global_position-owner_2D
	var direction=distance.normalized()
	if distance.length_squared()>max_distance:
		direction *=exp((-distance.length_squared()+max_distance)/(max_distance*8))
