...
func _ready():
	...
	,upgrade_pool[3],upgrade_pool[6],upgrade_pool[7],upgrade_pool[11],
	upgrade_pool[15],upgrade_pool[16],upgrade_pool[17]]#初始化选择池子,选择剑类的选项和所有解锁新武器的选项
...
func add_upgrade(upgrade:AbilityUpgrade):#这个字典控制已经有的能力
	...
	if upgrade.ID=="解锁战锤":
		can_chose_pool = can_chose_pool.filter(func(op): return op.ID != "解锁战锤")
		#删掉解锁战锤      
		for upgrade_option in upgrade_pool:
			
			if upgrade_option.Sort=="锤" and upgrade_option.ID!="解锁战锤":
