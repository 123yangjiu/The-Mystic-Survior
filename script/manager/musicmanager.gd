...
@onready var after_final: AudioStreamPlayer = $AfterFinal
@onready var the_real_final: AudioStreamPlayer = $TheRealFinal
...
func _on_after_final_finished() -> void:
	if no_continue:
		return
	play(after_final)
	is_second=true

func _on_the_real_final_finished() -> void:
	if no_continue:
		return
	play(the_real_final)
	is_second=true
