...
func on_timer_timeoout():
	...
	if player.direction.x>0:
		hammer_instance.get_node("AnimationPlayer").play("hammer_right")
	if player.direction.x<0:
		hammer_instance.get_node("AnimationPlayer").play("hammer_left")
	if player.direction.x==0:
		if randf()>0.5:
			hammer_instance.get_node("AnimationPlayer").play("hammer_left")
		else:
...
func on_ability_upgrade_add(upgrade:AbilityUpgrade,current_upgrade:Dictionary):
	...
	print("解锁成功")
	if upgrade.ID=="锤的速度":
		var persent_reduction=current_upgrade["锤的速度"]["quantity"]*.2
		$Timer.wait_time=base_wait_time*(1-persent_reduction)
		$Timer.start()
	if upgrade.ID=="锤的力量":
		var persent_improvement=current_upgrade["锤的力量"]["quantity"]*.2
