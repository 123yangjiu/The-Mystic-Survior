...
var all_threat_event:Dictionary[int,Callable]={}
#生成时与玩家的距离
var SPAWN_R=350
#地图边界尺寸
...
func _ready() -> void:
	...
	#添加固定事件
	all_special_event.append(single_appear)
...
func normal_special():
	...
	for enemy in enemy_manager.unlocked_group:
		if enemy.unlock_difficulty<gap+2 and enemy.ID!=EnemyUnlockEntry.ALL_ID.escape_box:
...
func line_front(enemy:EnemyUnlockEntry,wave:=2,wait_time:=2.5,distance:=450.0)->void:
	for i in wave:
...
func circle_interval(enemy:EnemyUnlockEntry,wave:=3,s_radius:=350.0,number:=20,wait_time:=0.1)->void:
	for j in wave:
		circle_surround(enemy,s_radius*(10+j*2)/10,number,wait_time)

func circle_surround(enemy:EnemyUnlockEntry,s_radius:=460.0,number:=80,wait_time:=0.0)->void:
	for i in number:
		var kill_enemy = enemy.scene.instantiate() as Node2D
		enemy_layer.add_child(kill_enemy)
		#半径全随机
		var angle := TAU/number
		# 2. 半径加 ±10% 随机，避免太机械
		var radius := float(s_radius) * randf_range(-s_radius/50.0,s_radius/50.0)
		# 3. 最终位置
		kill_enemy.global_position = GameEvent.play_global_position + Vector2.RIGHT.rotated(angle) * radius
		if wait_time!=0:
			await get_tree().create_timer(wait_time).timeout
